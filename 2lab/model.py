...
class Model:
    ...
    def run(self):
        query_gen = _generate(self._lambda)
        service_gen = _generate(self._mu)
        waiting_gen = _generate(self._nu)

        first_timepoint = next(query_gen)
        self._timeline.append(QueryArrived(first_timepoint))
        ...
        for event in self._timeline:
            ...
            if isinstance(event, QueryArrived):
                ...
                if has_available_terminals:
                    ...
                    query_processed_event = QueryProcessed(event.timepoint, query_processed_timepoint, terminal_no)
    ...
    def _show_theoretical_stats(self):
        ...
        p0 = 1 / (nsum(lambda k: ro ** k / fac(k), [0, self._n]) + (ro ** self._n / fac(self._n)) * nsum(
            lambda i: ro ** i / nprod(lambda l: (self._n + l * self._nu / self._mu), [1, i]), [1, self._m]))
        ...
        for s in range(1, self._m + 1):
            ps = ro ** (self._n + s) * p0 / (
                    fac(self._n) * nprod(lambda l: self._n + l * self._nu / self._mu, [1, s]))
            probs.append(ps)
    ...
    def show_stats(self):
        ...
        e_probs, e_p_denial, e_Q, e_A, e_avg_processing, e_avg_queue_length, e_avg_total, e_avg_processing_time, e_avg_waiting_time, e_avg_total_time = self._show_empirical_stats()
        print()
        t_probs, t_p_denial, t_Q, t_A, t_avg_processing, t_avg_queue_length, t_avg_total, t_avg_processing_time, t_avg_waiting_time, t_avg_total_time = self._show_theoretical_stats()
        ...
        ax[0].title.set_text(
            f'Empirical probabilities (n={self._n}, m={self._m}, lambda={self._lambda}, mu={self._mu}, nu={self._nu})')
        ax[0].hist(list(np.arange(0, len(e_probs), 1)), weights=e_probs)
        ax[1].title.set_text(
            f'Theoretical probabilities (n={self._n}, m={self._m}, lambda={self._lambda}, mu={self._mu}, nu={self._nu})')
        ax[1].hist(list(np.arange(0, len(t_probs), 1)), weights=t_probs)
        plt.show()
        self._show_stationary_stats()
        print('______________________________________________')
